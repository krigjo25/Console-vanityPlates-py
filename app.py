class VanityPlates():
    ...
    def PlateValidation(self, plate = str(input('Plate: ').upper())):

        #   Iterating throught the plate
        if self.CheckLengthAndSymbols(plate):
            if self.CheckTheNumbers(plate):

                return "Valid" if self.PlateDesign(plate) else f"{self.e}"
            

            else: return f"{self.e}"
